...
public class Parser {
    ...
    public Condition createCondition(String operator, String lhsAsString, String rhsAsString) {
        Expression lhs = parseExpression(lhsAsString);
        Expression rhs = parseExpression(rhsAsString);
        if(operator.equalsIgnoreCase("Greater") ||operator.equals(">") )
        {
            return new GreaterThanCondition(parseExpression(lhsAsString),parseExpression(rhsAsString));
        }else if (operator.equalsIgnoreCase("Less")||operator.equals("<"))
        {
            return new LessThanCondition(parseExpression(lhsAsString),parseExpression(rhsAsString));
        }else if (operator.equalsIgnoreCase("Equals")||operator.equalsIgnoreCase("=="))
        {
            return new EqualsCondition(parseExpression(lhsAsString),parseExpression(rhsAsString));
        }else {
            return null;
        }
        };
    ...
    public Expression createFunctionCallExpression(String functionName, List<String> parameterValuesAsStrings) {
        // TODO: Implement.
        List<Expression> local=new ArrayList<>();

        for(String s:parameterValuesAsStrings){
            local.add(parseExpression(s));
        }

        return new FunctionCallExpression(functionName,local);
    }
